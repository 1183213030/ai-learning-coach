...
```mermaid
graph TD
    INIT[1. INIT: 读取档案] --> REVIEW_GATE{2. REVIEW_GATE: 艾宾浩斯复习到期?}
    REVIEW_GATE -->|有到期项| SPOT_EXAM[REVIEW: 1~2 题快速抽测]
    SPOT_EXAM -->|通过| MAP[3. MAP: 5级能力梯]
    SPOT_EXAM -->|衰退| WEAK[打回 REPAIR ➔ 靶向修补]
    REVIEW_GATE -->|无到期项| MAP
    MAP --> CORE_20[4. CORE_20: 锁定20%高杠杆点]
    CORE_20 --> TUTOR[5. TUTOR: 最小充分解释 MCE]
    TUTOR --> PRACTICE[6. PRACTICE: 动态实战训练]
    PRACTICE --> EXAM[7. EXAM: 单题锁步考官]
    EXAM --> FEYNMAN[8. FEYNMAN: 概念穿透与抗误导]
    FEYNMAN --> SUMMARY[9. SUMMARY: 一页速查卡沉淀]
    SUMMARY --> ASSESS[10. ASSESS: 证据链终审验收]
    ASSESS -->|证据齐全| MASTERED[通关晋级 ➔ 纳入艾宾浩斯阶梯]
    ASSESS -->|存在短板| WEAK
    WEAK --> PRACTICE
```
...
| :--- | :--- | :--- | :--- |
| **1. INIT** | 档案初始化 | 提供学习目标、背景与可用时间 | 读取 `state/profile.md` 与学科状态，建立起点 |
| **2. REVIEW_GATE** | 艾宾浩斯复习门禁 | 独立作答到期知识点的极简抽测题 | 自动扫描 `next_review_due`，派发 1~2 题高浓度考核并动态调级 |
| **3. MAP** | 5级能力梯 | 确认或调整学习路线图 | 生成 L1~L5 能力梯，为每个知识点标注 `evidence_policy` |
| **4. CORE_20** | 核心点锁定 | 确认进入当前最核心的 20% 原子知识点 | 聚焦单一原子目标，屏蔽后续干扰信息 |
| **5. TUTOR** | 极简输入 (MCE) | 阅读核心机制白话与最小示例 (≤15行) | 给出最小充分解释，禁止长篇大论灌输 |
| **6. PRACTICE** | 动态实战 | 依据 6 种模式完成代码手写、推演或排错 | 动态调度 `BUILD` / `DEBUG` / `PREDICT` 等模式发起训练 |
| **7. EXAM** | 锁步考官 | 独立答题，给出心智推演或白板实现 | **单题锁步**，严格评分（PASS/SOUND/FLAW/FAIL），指出思维断层但不泄题 |
| **8. FEYNMAN** | 概念穿透 | 用大白话解释核心本质，或给出生活比喻 | 严查术语黑话（“穿透术语”），进行反向误导与极端并发压力测试 |
| **9. SUMMARY** | 速查沉淀 | 审阅并确认速查卡核心骨架与避坑清单 | 输出符合 `templates/cheat-sheet.md` 规范的 1 页速查卡 |
| **10. ASSESS** | 证据链终验 | 查验证据达成情况 | 清点 E1~E5 达成情况，通关后登记艾宾浩斯阶梯 (`Stage 1`) |
...
### `/review` —— 主动唤醒艾宾浩斯复习
- **场景**：想在学新课前集中巩固已学知识，或查看遗忘风险清单。
- **行为**：AI 列出当前学科所有掌握知识点的阶梯分布（Stage 1~6）与到期状态，并立即启动抽测考核。

### `/skip` —— 快速跳关测试
- **场景**：当前知识点你已经非常熟练，想直接跳过。
- **行为**：AI 立即生成 1 道**综合盲打题**（覆盖白板手写与边界条件）。作答通过（PASS）直接全量点亮证据链并跳关；未通过则直接驳回并进入靶向修补。

### `/status` —— 打印进度简报
- **场景**：查看当前学习状态与证据达成进度。
- **行为**：输出当前 Stage、当前原子目标、E1~E5 证据点亮情况、复习到期预警以及薄弱项清单。

### `/exit` —— 结课与归档
- **场景**：本次学习结束准备休息。
- **行为**：自动生成本次学习日志（包含复习抽测记录与证据链），更新学科状态并输出下次启动建议。
